...
#One to count the number of different error messages
# The other to count the number of entries for each user (splitting between INFO and ERROR types).

error = {}
per_user = {}

#Parese through all log entries found in syslog.log file by iterating over the file.
...
with open("syslog.log") as f:
    for line in f:
        ...
        #check if entry matches the INFO or ERROR message formats.
        if "INFO" in line:
...
#sort error dictionary from most common to least common error
